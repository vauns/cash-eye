...
"""
    ...
    if success:
        ...
        logger.info("")
        logger.info("后续步骤:")
        logger.info("1. 构建离线 Docker 镜像:")
        logger.info("   docker build -f Dockerfile.offline -t money-ocr-api:offline .")
        logger.info("")
        logger.info("2. 或使用构建脚本:")
        logger.info("   bash scripts/build_offline_image.sh")
        logger.info("   或 (Windows): scripts\\build_offline_image.bat")
        sys.exit(0)
    else:
        logger.error("")
        logger.error("✗ 模型复制失败")
